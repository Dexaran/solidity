...
contract token is safeMath {
    ...
    // [-TESTING EVENTS]
    uint emulatedSupply;
    mapping (address => uint) emulatedBalance;
    
    
    function DEBUG_setSupply(uint s) {
        emulatedSupply = s;
    }
    function DEBUG_setBalance(address a, uint b) {
        emulatedBalance[a] = b;
    }
    ...
    */
    ...
    function _processTransactionFee(address addr, uint256 value) internal {
        ...
        */
        //if ( isICO ) { return; }
        uint256 fee = getTransactionFee(value); //TODO
       ...
       /* if ( schellingAddr != 0x00 ) {
        ...
        burn_(addr, forBurn);
        bytes memory data;
        address schellingAddr = 0xeeee1111e65703f0076e4812653aab85fca0f0;
        Transfer(addr, schellingAddr, forSchelling, data);
    }
    ...
    function configure() isReady external returns(bool) {
        return true;
    }
